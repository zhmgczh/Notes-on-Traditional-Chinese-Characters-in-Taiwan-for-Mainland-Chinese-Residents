...
add_filter('posts_clauses', function ($clauses, $query) {
    ...
    // 把搜尋字串切成 terms（你也可以改成只取第一個詞）
    $terms = preg_split('/\s+/u', $s, -1, PREG_SPLIT_NO_EMPTY);
    if (!$terms) return $clauses;

    // 生成「keyword 在引號內」的 REGEXP pattern
    // 注意：MySQL REGEXP 是 ERE 風格；這裡用比較保守的 escaping
    $patterns = [];
    foreach ($terms as $t) {
        $t = trim($t);
        if ($t === '') continue;
        ...
        // 逃逸 regex 特殊字元
        $re = preg_quote($t, '/');
        // MySQL REGEXP 也把 backslash 當 escape，所以再多逃逸一次
        $re = str_replace('\\', '\\\\', $re);

        // 只判斷「」內是否包含該 term： 「[^」]*term[^」]*」
        $patterns[] = "「[^」]*{$re}[^」]*」";

        // 如果你也有用英文半形引號 " "，可一起加入（可刪）
        $patterns[] = "\"[^\"]*{$re}[^\"]*\"";

        // 如果你也會用中文弯引號 “ ”，可一起加入（可刪）
        $patterns[] = "“[^”]*{$re}[^”]*”";
    }

    if (!$patterns) return $clauses;

    // 權重：每命中一個 pattern 加 100（可調）
    $caseSqlParts = [];
    $caseArgs = [];
    foreach ($patterns as $p) {
        $caseSqlParts[] = "CASE WHEN {$wpdb->posts}.post_title REGEXP %s THEN 100 ELSE 0 END";
        $caseArgs[] = $p;
    }

    $scoreSql = '(' . implode(' + ', $caseSqlParts) . ')';

    // 在 SELECT 加一個 score 欄位
    $clauses['fields'] .= ", {$wpdb->prepare($scoreSql, ...$caseArgs)} AS quoted_kw_score";

    // 排序：先看引號內命中分數，再用原本排序（你也可改成按相關度/日期等）
    $clauses['orderby'] = "quoted_kw_score DESC, {$wpdb->posts}.post_date DESC";
